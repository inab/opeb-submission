...
sshpass -p first sftp -P 8222 -F /dev/null -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o BatchMode=no -b - first@localhost << EOF > "$TMP_DIR/sftp_upload_test_digest.txt"
	cd submissions/one
